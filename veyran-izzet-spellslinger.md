# Veyran, Voice of Duality — Izzet Spellslinger

2026-09-17
**Updated 2026-09-18 / 09-19** across earlier passes (trim to 100, Ral + Aetherflux, Force of Will + Steam Vents).
**Updated 2026-09-20.** No list changes this round — the 100 is unchanged. Regenerated with an explicit tutor-by-board-state guide, and with one card-conflict correction: **Steam Vents and Shivan Reef are now shared with the Sauron deck**, so the earlier "not shared with the other three decks" line no longer holds. Deck stays at 100, 34 lands, three Game Changers (the Bracket 3 cap).
**Updated 2026-09-21.** One swap: **Terminal Moraine → Spirebluff Canal.** This takes the deck's own "weakest slot" flag off the board — a colorless sac-land becomes an Izzet fast land that enters untapped early and makes both colours. Red ticks up to 19, the colorless-only count drops to one (Scavenger Grounds), and the land count is unchanged at 34. Not a Game Changer, so the deck stays at 100 and three GCs (Bracket 3, at the cap).
**Updated 2026-09-22.** One swap: **Scavenger Grounds → Stormcarved Coast.** This clears the deck's last flagged weak slot: a colorless land whose graveyard exile fought Eris, Mizzix's Mastery, and Plunder the Trollshaws becomes an Izzet slow land (enters untapped once you control two other lands) that makes both colours. Blue goes to 24 sources, red to 20, and the deck now has no colorless-only lands. The card-conflict section is also rebuilt against the other three decks' current lists. Not a Game Changer, so the deck stays at 100, 34 lands, and three GCs (Bracket 3, at the cap).

## Strategy

Chain cheap instants and sorceries, let Veyran make every cast-trigger fire twice, and convert those doubled triggers into damage, fliers, or cards.

Veyran ({1}{U}{R}, 2/2) reads: magecraft gives it +1/+1 until end of turn, and *if you casting or copying an instant or sorcery causes a triggered ability of a permanent you control to trigger, that ability triggers an additional time*.

The scope matters. Veyran doubles magecraft and "whenever you cast an instant or sorcery" abilities. It does **not** double attack triggers, combat-damage triggers, or ETB triggers — so Kylox's attack ability, the Emeritus attack triggers, and Lightning's damage trigger all fire once.

| Payoff | Per spell | With Veyran | Wizard/Shaman (Harmonic Prodigy too) |
| --- | --- | --- | --- |
| Guttersnipe | 2 damage to each opponent | 4 | 6 |
| Thunderdrum Soloist | 1 damage each opponent (3 if 5+ mana spent) | 2 (6) | no |
| Ral, Storm Conduit | 1 damage to a target on cast | 2 | no (planeswalker) |
| Aetherflux Reservoir | gain life = spells cast this turn | that lifegain fires twice | no (artifact) |
| Archmage Emeritus | draw 1 | draw 2 | draw 3 |
| Murmuring Mystic | one 1/1 flying Bird | 2 Birds | 3 Birds |
| Rite of the Dragoncaller | one 5/5 flying Dragon | 2 Dragons | no |
| Eris, Roar of the Storm | one 4/4 flying prowess Dragon on your 2nd spell | 2 Dragons | no |
| Burning Prophet | +1/+1 and scry 1 | twice | three times |
| Displacer Kitten | blink one of your permanents | 2 blinks | no |

Thor, Asgard's Avenger adds 1 to every instance of damage from another source you control. With Veyran it turns each Guttersnipe trigger into 3 and each Thunderdrum trigger into 2, so a three-spell turn kills a 40-life table faster than the raw numbers suggest.

**Ral, Storm Conduit** is the third pinger. He deals 1 to any target on every instant or sorcery you cast (2 under Veyran), and he gains loyalty whenever you copy or double a spell, so he protects and grows himself.

**Aetherflux Reservoir** is the burst finisher and combo payoff. Its lifegain is a "whenever you cast a spell" trigger on a permanent you control, so Veyran doubles it on every instant or sorcery: each cast gains life equal to the current spell count, twice. A dense turn crosses 50 life quickly, at which point pay 50, deal 50 to a player.

**Ways to close.** Burn the table with Guttersnipe, Thunderdrum Soloist, and Ral over two or three spell-dense turns. Build an air force with Rite of the Dragoncaller and Eris, then swing. Point Crackle with Power at a player after a Seething Song burst. Or storm off into Aetherflux for a life-into-burn snipe.

Eris is the reward for a full graveyard: it costs {2} less per *different* mana value among instants and sorceries there, so a yard with four mana values makes it a 2-mana 4/4 flier.

## Consistency

The list is exactly 100 cards.

34 lands (19 nonbasic, 9 Island, 6 Mountain) plus four rocks: Sol Ring, Arcane Signet, Fellwar Stone, Thought Vessel. (Aetherflux Reservoir is an artifact but not a mana source.)

| Measure | Count | Note |
| --- | --- | --- |
| Blue lands | 24 | 9 Island plus the duals (incl. Steam Vents, Spirebluff Canal, Stormcarved Coast), Command Tower, Exotic Orchard, Path of Ancestry |
| Red lands | 20 | 6 Mountain plus the duals (incl. Steam Vents, Spirebluff Canal, Stormcarved Coast) and Sokenzan |
| Colorless-only lands | 0 | — |
| No mana the turn they land | 3 | Ash Barrens, Evolving Wilds, Terramorphic Expanse |
| Usually enter tapped | 5 | Coastal Peak, Temple of Epiphany, Path of Ancestry, Scorched Geyser, Mistrise Village (Stormcarved Coast only on turns one and two) |

Red is still the tighter color, but 20 red lands is healthier than the old 18 for Thor ({2}{R}{R}) and for a Crackle with Power big enough to matter. Two-mana rocks are thin — only Arcane Signet and Fellwar Stone fix on turn two — so the untapped duals carry early fixing, and Spirebluff Canal joins Steam Vents as a source that reliably enters untapped in the opening turns.

**Spell density.** 36 instants and sorceries, plus four prepare cards whose backs are castable spells: Emeritus of Ideation // Ancestral Recall, Emeritus of Conflict // Lightning Bolt, Blazing Firesinger // Seething Song, Sanar // Wild Idea. Each copy is a separate cast, so each triggers magecraft again.

**Finding cards.** Three repeatable draw bodies (Archmage Emeritus, Muse Seeker, Hydro-Channeler) and a deep cantrip suite: Sleight of Hand, Think Twice, Deduce, Quick Study, Dreams of Laguna, Light Up the Stage, Expressive Iteration, Flow State, Stock Up. Plunder the Trollshaws is the best of them here, since flashback turns one card into two casts. Old Fat Spider Can't See Me gives Veyran hexproof on chapter I, then two cards on III and IV.

**Interaction.** Eight ways to answer a spell (Counterspell, Force of Will, Spell Pierce, An Offer You Can't Refuse, Disdainful Stroke, Mystic Confluence, Return the Favor, Venser), nine pieces of spot removal, and three resets in Blasphemous Act, River's Rebuke, and Cyclonic Rift. Every one is also a magecraft trigger, which is why holding up interaction costs this deck no tempo. Force of Will is the free one — it protects your commander or your combo turn even fully tapped out, and it's live whether or not Veyran is on the board.

**Redundancy gap, narrowed.** Three recurring damage-on-cast sources (Guttersnipe, Thunderdrum Soloist, Ral, Storm Conduit) and one graveyard rebuy (Mizzix's Mastery). Losing one pinger no longer turns the burn plan off.

**Fragility.** Veyran is a 2/2 with no evasion and no protection of its own, and the magecraft pump lasts only until end of turn. Hold Force of Will, Return the Favor, or a Mistrise Village activation for the turn you commit. Force of Will is best because it's free and doesn't need Veyran alive; Old Fat Spider's chapter I gives a one-turn hexproof window.

## Key lines

**Displacer Kitten plus Blazing Firesinger is an infinite loop.** Firesinger enters prepared, so you may cast a copy of Seething Song ({2}{R}: add {R}{R}{R}{R}{R}) for a net two red mana. That copy is a noncreature spell, so Displacer Kitten blinks Firesinger, it enters prepared again, and you repeat. Infinite red mana and infinite instant casts, so infinite magecraft triggers.

With Guttersnipe or Thunderdrum Soloist out, that loop is lethal. With Rite of the Dragoncaller it makes arbitrarily many 5/5 fliers. With Archmage Emeritus it draws your deck, so stop while cards remain. **With Aetherflux Reservoir it gains infinite life, so pay 50 and deal 50 to each opponent in turn — the cleanest kill the deck has.** Assembly costs seven mana across two turns, plus three mana free to start the loop.

**Kylox is the other big turn.** Attack, sacrifice your Birds and Dragons, exile that much off the top, and cast every instant and sorcery among them for free. Each free cast is a doubled magecraft trigger — and each also pings with Ral and gains with Aetherflux. Kylox's own attack trigger is not doubled.

**Mizzix's Mastery** on River's Rebuke or a large Crackle with Power is the single-card blowout. Dualcaster Mage is value here rather than a combo piece, since Twinflame is no longer in the list.

**Sequencing.** Deploy pingers and token engines before Veyran, not after. Veyran costs three, dies to anything, and only doubles what is already on the board. Ral, Storm Conduit and Aetherflux Reservoir both want to be down before the spell-dense turn. Emeritus of Ideation wants to exile eight cards from your graveyard on attack, which competes with Eris and Mizzix's Mastery for the same yard, so feed it lands and spent cantrips.

## Tutor targets by board state

Three tutors, each pointing somewhere different, plus one graveyard rebuy — and the key limit is that **Mystical Tutor and Solve the Equation only find instants and sorceries**, so neither can fetch the combo pieces (Displacer Kitten is a creature, Blazing Firesinger is a creature, Aetherflux Reservoir is an artifact). Fabricate is your only bridge to the artifact half.

- **Loop assembled but no payoff → Fabricate for Aetherflux Reservoir.** Fabricate is the only tutor that reaches it, and Aetherflux turns infinite casts into an infinite-life-into-burn kill. It also finds Disruptor Flute when you want proactive interaction instead.
- **Need to protect a kill turn → Solve the Equation for a counter** (to hand, castable this turn — Counterspell, Return the Favor, Disdainful Stroke), or **Mystical Tutor at an opponent's end step** for an answer you'll draw into on your turn. Force of Will you'd rather already hold, since it's free.
- **Need reach to close → Mystical Tutor or Solve the Equation for Crackle with Power** after a mana burst, or **Mizzix's Mastery** to recast River's Rebuke or a big Crackle out of the graveyard.
- **Need to dig / rebuild the hand → any cantrip, or a draw spell** (Stock Up, Dreams of Laguna) via either instant/sorcery tutor.
- **Mana-screwed → Fabricate for Sol Ring or Thought Vessel**, the only tutorable fix (the other rocks aren't reachable by the spell tutors).

Timing note: **Mystical Tutor puts the card on top of your library** — you draw it next, so it "costs" a draw. Fire it at the end of the turn before you go off, or convert it immediately with a Mystic Sanctuary shuffle or an Archmage Emeritus draw. **Solve the Equation goes to hand**, so it's the better pick when you intend to cast the card this turn.

## Bracket and Rule 0

Bracket 3, at the cap with **three** Game Changers: **Mystical Tutor**, **Cyclonic Rift**, and **Force of Will**. A fourth moves the deck to Bracket 4. Ral, Storm Conduit and Aetherflux Reservoir are both off-list.

Disclose before the game:

- **Displacer Kitten plus Blazing Firesinger** is a two-card infinite. It needs seven mana of setup, so it's not an early combo, but it is a combo and pods should hear about it.
- **Aetherflux Reservoir plus that loop is a combo kill** — infinite casts becomes infinite life, then pay 50 and burn each opponent. Aetherflux also snipes off a big non-infinite turn once you pass 50 life, so mention it as a wincon in its own right.
- **Cyclonic Rift**, **Force of Will** (a free counterspell — worth flagging as a Game Changer), and **Jin-Gitaxias, Progress Tyrant**, which taxes or answers the first spell each opponent casts.
- **Kylox** turns, which can convert a board of tokens into a long free-spell chain.

Re-check the official list at https://commanderbrackets.com/faq before an event.

## Decklist by type (100)

**Commander (1).** Veyran, Voice of Duality

**Planeswalkers (1).** Ral, Storm Conduit

**Creatures (20).** Archmage Emeritus, Blazing Firesinger // Seething Song, Burning Prophet, Displacer Kitten, Dualcaster Mage, Emeritus of Conflict // Lightning Bolt, Emeritus of Ideation // Ancestral Recall, Eris Roar of the Storm, Gandalf Friend of the Shire, Guttersnipe, Harmonic Prodigy, Hydro-Channeler, Jin-Gitaxias Progress Tyrant, Kylox Visionary Inventor, Murmuring Mystic, Muse Seeker, Sanar Unfinished Genius // Wild Idea, Thor Asgard's Avenger, Thunderdrum Soloist, Venser Shaper Savant

**Artifacts (6).** Aetherflux Reservoir, Arcane Signet, Disruptor Flute, Fellwar Stone, Sol Ring, Thought Vessel

**Enchantments (2).** Old Fat Spider Can't See Me, Rite of the Dragoncaller

**Instants (25).** Abrade, An Offer You Can't Refuse, Banishing Betrayal, Burst Lightning, Chaos Warp, Counterspell, Cyclonic Rift, Deduce, Disdainful Stroke, Dreams of Laguna, Flashback, Force of Will, Lightning Bolt, Mystic Confluence, Mystical Tutor, Plunder the Trollshaws, Pongify, Prismari Charm, Quick Study, Reality Shift, Reenact the Crime, Return the Favor, Spell Pierce, Think Twice, Vibrant Outburst

**Sorceries (11).** Blasphemous Act, Crackle with Power, Expressive Iteration, Fabricate, Flow State, Light Up the Stage, Mizzix's Mastery, River's Rebuke, Sleight of Hand, Solve the Equation, Stock Up

**Lands (34).** Ash Barrens, Coastal Peak, Command Tower, Desolate Lighthouse, Evolving Wilds, Exotic Orchard, Mistrise Village, Mystic Sanctuary, Path of Ancestry, Scorched Geyser, Shivan Reef, Sokenzan Crucible of Defiance, Spectacle Summit, Spirebluff Canal, Steam Vents, Stormcarved Coast, Sulfur Falls, Temple of Epiphany, Terramorphic Expanse, 9 Island, 6 Mountain

## Import list (100)

```
1 Abrade
1 Aetherflux Reservoir
1 An Offer You Can't Refuse
1 Arcane Signet
1 Archmage Emeritus
1 Ash Barrens
1 Banishing Betrayal
1 Blasphemous Act
1 Blazing Firesinger // Seething Song
1 Burning Prophet
1 Burst Lightning
1 Chaos Warp
1 Coastal Peak
1 Command Tower
1 Counterspell
1 Crackle with Power
1 Cyclonic Rift
1 Deduce
1 Desolate Lighthouse
1 Disdainful Stroke
1 Displacer Kitten
1 Disruptor Flute
1 Dreams of Laguna
1 Dualcaster Mage
1 Emeritus of Conflict // Lightning Bolt
1 Emeritus of Ideation // Ancestral Recall
1 Eris, Roar of the Storm
1 Evolving Wilds
1 Exotic Orchard
1 Expressive Iteration
1 Fabricate
1 Fellwar Stone
1 Flashback
1 Flow State
1 Force of Will
1 Gandalf, Friend of the Shire
1 Guttersnipe
1 Harmonic Prodigy
1 Hydro-Channeler
1 Jin-Gitaxias, Progress Tyrant
1 Kylox, Visionary Inventor
1 Light Up the Stage
1 Lightning Bolt
1 Mistrise Village
1 Mizzix's Mastery
1 Murmuring Mystic
1 Muse Seeker
1 Mystic Confluence
1 Mystic Sanctuary
1 Mystical Tutor
1 Old Fat Spider Can't See Me
1 Path of Ancestry
1 Plunder the Trollshaws
1 Pongify
1 Prismari Charm
1 Quick Study
1 Ral, Storm Conduit
1 Reality Shift
1 Reenact the Crime
1 Return the Favor
1 Rite of the Dragoncaller
1 River's Rebuke
1 Sanar, Unfinished Genius // Wild Idea
1 Scorched Geyser
1 Shivan Reef
1 Sleight of Hand
1 Sokenzan, Crucible of Defiance
1 Sol Ring
1 Solve the Equation
1 Spectacle Summit
1 Spell Pierce
1 Spirebluff Canal
1 Steam Vents
1 Stock Up
1 Stormcarved Coast
1 Sulfur Falls
1 Temple of Epiphany
1 Terramorphic Expanse
1 Think Twice
1 Thor, Asgard's Avenger
1 Thought Vessel
1 Thunderdrum Soloist
1 Venser, Shaper Savant
1 Veyran, Voice of Duality
1 Vibrant Outburst
9 Island
6 Mountain
```

## Flags and tuning levers

**The 09-21 swap — 1 out, 1 in (both lands).** Terminal Moraine → Spirebluff Canal. This is the "Terminal Moraine for a third untapped dual" upgrade the old notes flagged, now taken: a colorless land that entered slow and sacked for a basic becomes an Izzet fast land that enters untapped through the early turns and taps for both colours. Red moves to 19, colorless-only lands drop to one, land count holds at 34. Not a Game Changer.

**Weakest remaining slot**, if you want to keep tuning: with Scavenger Grounds gone, the mana base has no obvious weak land left. On the spell side, Disruptor Flute and Dualcaster Mage (value-only since Twinflame left) are the softest.

**Game Changers — at the cap.** Three: Mystical Tutor, Cyclonic Rift, Force of Will. A fourth moves the deck to Bracket 4. Force of Will earns the slot over Fierce Guardianship (free only while you control your commander — shaky on a 2/2 that dies constantly) and Rhystic Study (generic flow that doesn't touch the magecraft engine); Force of Will is free unconditionally and doesn't need Veyran alive.

**Rules notes carried forward.** Veyran has no first strike, and magecraft gives +1/+1 until end of turn, not permanent counters. The doubling applies only when your casting or copying an instant or sorcery is what caused the trigger.

**Steam Vents, Coastal Peak, and Scorched Geyser** are all Island Mountain duals with red-blue identity, so all three are clean Izzet fixing and all three turn on Sulfur Falls. Steam Vents is the best — untapped whenever you'll pay 2 life. Coastal Peak always enters tapped and cycles for {2}; Scorched Geyser enters untapped once you control two basics.

**Graveyard tension resolved.** Scavenger Grounds was the only card that exiled your own graveyard; with it gone, Eris, Mizzix's Mastery, and Plunder the Trollshaws flashback no longer have an in-deck enemy.

**Card conflicts with your other decks.**
- Displacer Kitten stays here — it's half the infinite loop, so the physical copy lives in Veyran.
- Shared with **Sauron**: Blasphemous Act, Counterspell, Cyclonic Rift, Shivan Reef, Steam Vents, Sulfur Falls. (Scavenger Grounds is now Sauron-only.)
- Shared with **Najeela**: Fellwar Stone, Temple of Epiphany, Terramorphic Expanse.
- Shared with **Najeela and Sauron**: Chaos Warp, Path of Ancestry.
- Shared with **Sephiroth and Najeela**: Ash Barrens, Evolving Wilds.
- Staples in all four decks: Arcane Signet, Command Tower, Exotic Orchard, Sol Ring.
- **Stormcarved Coast** is a new single, not shared with any other deck.

**If you want red to be more reliable,** Spirebluff Canal and Stormcarved Coast have bumped it to 20. The next step is bringing back Izzet Signet as a two-mana rock that also fixes red.
