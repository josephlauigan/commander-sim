...
Follows Scryfall's API guidelines (https://scryfall.com/docs/api):
  ...
  - cache results instead of re-downloading (scryfall_cache.json next to this file)

Usage from the command line:
    python3 scryfall.py "Lightning Bolt" "Talrand, Sky Summoner"     # fetch + show
    python3 scryfall.py --deck veyran                                 # fetch a whole deck list
"""
import json, os, sys, time, urllib.request, urllib.parse, urllib.error

HERE = os.path.dirname(os.path.abspath(__file__))
CACHE_PATH = os.path.join(HERE, 'scryfall_cache.json')
