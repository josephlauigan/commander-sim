"""Test proposed list changes for an opponent-pool deck without editing the list.
    ...
    python3 tools_swaptest.py <deck key> <tier the deck plays against> <games> "Out Card>In Card; Out>In; ..."
...
import engine as E, ais, compare, pools, poolmode
...
def _run(args):
    ...
    import cards as C; C.ensure_cards(sorted(set(cards)), verbose=False)
    out = []
    for s in seeds:
