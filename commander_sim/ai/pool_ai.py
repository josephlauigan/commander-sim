...
"""
import importlib
from commander_sim.engine import has, total_mana, pval
...
def _options(p, m):
    ...
    for x in p.perms:
        if x.cd is not None and x.cd.name in PROTECTORS and PROTECTORS[x.cd.name][0] != 'hand' and not x.phased \
                and not importlib.import_module('commander_sim.cards.impl.rules').ability_locked(E.CUR_G, x, p):
                ...
                if tg:
                    ...
                    if pval(g, foe) >= 3:
                        def might(c=c, me=me, foe=foe):
                            ...
                            from commander_sim.cards import cardimpl; cardimpl._eot(g, me, x, x)
