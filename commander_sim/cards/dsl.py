...
from commander_sim.engine import (has, army_of, epow, etgh, pval, threat, lose_life, gain, draw, mill, amass, make_tokens,
...
from commander_sim.cards.dsl_parse import compile_card
...
def ability_options(g, p, sorcery_ok=True):
    ...
    for src in list(p.perms):
        ...
        if importlib.import_module('commander_sim.cards.impl.rules').ability_locked(g, src, p): continue
...
OVERRIDES_PATH = os.path.join(__import__('commander_sim').DATA, 'cards_dsl.json')
...
if __name__ == '__main__':
    from commander_sim.cards import scryfall, autotag
