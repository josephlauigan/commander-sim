...
from commander_sim.cards.impl.common import best_opp_creature, best_opp_nonland, make_artifact_tokens
...
def _dig(g, p, c, ctx):
    delve_exile(g, p, c, delve_count(g, p, c))
    from commander_sim.cards.impl import topdeck as T
    top = T.look(p, 7)
...
CI.SPELL_PRIO['Draco'] = lambda g, p, c: 0 if importlib.import_module('commander_sim.cards.impl.topdeck').mv_mode(p) else 50
...
def etb_val(m):
    return importlib.import_module('commander_sim.cards.dsl').etb_value(m.cd) if m.cd is not None and m.cd.dsl else (2.0 if m.cd is not None and 'etb' in m.cd.tags else 0.0)
...
# ------------------------------------------------------------------ Lim-Dûl's Vault
@IC.spell("Lim-Dûl's Vault", prio=lambda g, p, c: 45 if importlib.import_module('commander_sim.ai.pool_ai').wish_list(g, p) else 0, types='I',
...
def _whirler(g, src, p, atk, d):
    ...
    from commander_sim.cards.impl import t4 as impl_t4
