...
status of each deck's key engines (from its '## Sim modeling notes').
    ...
    python3 -m commander_sim.pool_audit                    # summary table per deck + engine status
    python3 -m commander_sim.pool_audit --deck yuriko      # every card of one deck (key prefix is enough)
    python3 -m commander_sim.pool_audit --md FILE          # write the tables as Markdown
    python3 -m commander_sim.pool_audit --mine             # your decks: every card that is not Full
