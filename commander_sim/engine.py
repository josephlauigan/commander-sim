...
def untargetable(g, m):
    ...
    if equipped(m, 'helm') and CI is not None and importlib.import_module('commander_sim.cards.impl.mine').is_legendary(g, m): return True   # Champion's Helm
...
def mana_units(g, p, convoke=False):
    ...
    for m in p.perms:
        ...
        if 'rock' in t:
            ...
            if m.cd.name == 'Delighted Halfling': c = importlib.import_module('commander_sim.cards.impl.rules').halfling_colors(p) or 'C'
...
def die(g, m, cause='destroy'):
    ...
    if g.hooks and CI.total(g, 'no_graveyard', p):          # Rest in Peace: exiled, so it never dies
        ...
        return
    if m.creature and CI is not None and importlib.import_module('commander_sim.cards.impl.partials').hushed(g):
        leave(g, m)
...
def resolve(g, p, c, ctx, zone):
    ...
    if 'fill' in t: from commander_sim import ais; ais.seph_fill_resolve(g, p, t['fill'], ctx)
    if 'rean' in t: from commander_sim import ais; ais.seph_rean_resolve(g, p, c, ctx)
    if 'burn' in t:
        opps = g.opps(p)
...
def add_treasure(g, p, n=1):
    ...
    if n <= 0: return
    if CI is not None: importlib.import_module('commander_sim.cards.impl.common').make_artifact_tokens(g, p, 'Treasure', n)
    else: p.treasures += n
...
def apply_removal(g, actor, m, kind, spell=None):
    ...
    ward = (m.cd.ward if m.cd is not None else 0) + importlib.import_module('commander_sim.cards.impl.rules2').aura_ward(g, m)
