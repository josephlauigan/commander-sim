...
from commander_sim import engine, ais
...
def check_identity(deck, swaps):
    from commander_sim.cards import sources as cards
    from commander_sim.engine import IDENT
...
def report_errors():
    ...
    print('  Replay one to see what happened:  python3 -m commander_sim --deck <deck> [--swap ...] --trace <seed - 500000>')
...
def main():
    ap = argparse.ArgumentParser(prog='python3 -m commander_sim', description='Commander pod simulator: one of your decks against three outside decks '
...
def cli():
    """the command-line entry point (python3 -m commander_sim, or python3 -m commander_sim.compare)"""
    try:
        main()
    finally:
