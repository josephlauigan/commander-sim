...
The commander comes from the '**Commander:**' line.
    ...
    python3 -m commander_sim.pools                 # list the pools
    python3 -m commander_sim.pools --validate      # structure, card names, colour identity, bans, Game Changers per tier
"""
import collections, glob, os, random, re, sys
from commander_sim.decks import load

from commander_sim import ROOT
POOL_DIR = os.path.join(ROOT, 'decklists', 'pool')
...
def register(decks=None, verbose=False):
    ...
    from commander_sim.cards import sources as cards, scryfall, cardimpl, pool_cards
    from commander_sim.ai import pool_ai, pool_decks
    cardimpl.load()
