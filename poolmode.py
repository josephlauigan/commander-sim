...
a deck (a --swap) face identical opponents and identical opponent draws: paired, common-random-number runs.
    ...
    python3 compare.py --deck seph --pool t3 --games 10000
    python3 compare.py --deck seph --pool t3 --swap "Persist=>Dread Return"      # paired A/B
    python3 compare.py --all-decks --pool all --games 10000 --jobs 24            # deck x tier matrix
    python3 compare.py --deck seph --pool t3 --analyze
    python3 compare.py --calibrate within|ordering --games 10000                 # pool balance checks
"""
import math, sys, time
from collections import Counter, defaultdict
import engine, ais, pools
import compare as C
...
def _setup(profile, ai, temp, extra_cards=()):
    ...
    pools.register()
    if extra_cards:
        import cards as _cards; _cards.ensure_cards(list(extra_cards), verbose=False)
